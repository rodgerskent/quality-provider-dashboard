...
console.log("Violation Descriptions from json file")
console.log(vDescriptions)

// The program then sets the initial landing page based on the city of Aurora
d3.csv("./inspectionData.csv").then(function (diningData) {
  ...
  if (inputValue1) {
    // Clear the list of shame table body
    var tbody = d3.select("tbody");
    tbody.html("");

    // Present a list of restaurants for the list of shame
  ...
  console.log("Finished initial load = map, bar, pie, list of shame & drop menus")
...
// --------------------------------------------------------------------------------------------
// 2nd Section just to update the table body with violation details ... and return to list of shame
...
function shameChanged() {
  d3.csv("./inspectionData.csv").then(function (tttData) {
    ...
    if (inputValue4 != "List of Shame") {
      console.log("Down the restaurant rabbit hole")
      console.log("InputValue4 ...")
      console.log(inputValue4)
      var tttRestaurant = tttEstablishment.filter(item => item["Program Identifier"] === inputValue4)[0];
      console.log("tttRestaurant ...")
      ...
      var tbody = d3.select("tbody");
      tbody.html("");
      // Build a list of the inspection violations and join with the json descriptions
    ...
    if (inputValue4 === "List of Shame") {
      console.log("Down the reset restaurant list rabbit hole")
      //var inputElement1 = "Aurora"
      var inputElement1 = d3.select("#selDataset1");
      var inputValue1 = inputElement1.property("value");
      console.log("inputValue1 ...")
      console.log(inputValue1)
      ...
      // Clear the table body of violations to make room for restaurant list
        var tbody = d3.select("tbody");
        tbody.html("");
        var targetCity2 = tttEstablishment.filter(item => item.City === inputValue1);
        var updateShame = targetCity2.filter(item => item["Total Foodborne Illness Risk Violations"] >= 5)
...
// ------------------------------------------------------------------------
// 3rd Section that responds to user selections on city and premise type
var form = d3.select("form");
form.on("submit", optionChanged);

// This function kicks off when the user selects a different city or premise type
function optionChanged() {
  d3.csv("./inspectionData.csv").then(function (targetData) {
    // Clears the drop down menu to make way for new city update
    ...
    // This updates the list of shame for the selected city and presents in table
    function fnShame(a, b) {
      if (a > b) {
        return -1;
      }
    ...
    // Clears the earlier table items to make room for updated list
    var tbody = d3.select("tbody");
    tbody.html("");
    ...
    // Adds new drop down menu restaurant list based on the city the user selected
    ...
    // Adds the "List of Shame" heading so user can return to summarized level
    option.insert("option",":first-child").text("List of Shame");
      

    // Replaces list of shame with the detailed violations
    console.log(vDescriptions)
    var tbody = d3.select("tbody");
    tbody.html("");
    var inputElement4 = d3.select("#selDataset4");
